...
All the previous ways of communication work in real time and rely in Figure~\ref{fig:RTC} topology but with different technologies. In this thesis we will manly work with Internet RTC using media and data.
...
Figure~\ref{fig:mediastreamAPI} illustrates how the browser access the media and delivers the output to JavaScript. We will use this function to build WebRTC enabled applications for RTC video conferencing. The video tag is an HTML5 a Document Object Model (DOM) \nomenclature{DOM}{Document Object Model} element that reproduces local and remote media streams.

{\it GetUserMedia} API works using a fallback model, this means than the JavaScript method will return an object that can be played in an HTML web application, a simple example of this method can be seen in the following code.
...
Signaling messages will be exchanged using a topology similar to Figure~\ref{fig:webrtcExample}, with the messages being sent either by WebSockets or other similar HTTP polling protocols. Messages are built using a modified bundled version of SDP, WebRTC signaling messages are similar to SIP as they use SDP bodies for the agreement.
...
To access the statistical data retrieved from the control messages we need to use the {\it getStats()} method in the {\it PeerConnection}, this method will allow the developers to access that data in a JSON format that might require some post-processing. Statistical models will be useful for the developers to monitor the status of their WebRTC applications and adapt the attributes of the {\it PeerConnection}.
...
Quality of Service (QoS) \nomenclature{QoS}{Quality of Service} for WebRTC is also being discussed in the IETF and a draft is available with some proposals~\cite{qosWebRTCIETF}. WebRTC uses DiffServ packet marking for QoS but this is not sufficient to help prevent congestion in some environments. When using DiffServ, the problem arises from the Internet Service Providers (ISPs) as they might be using their own packet marking with different DiffServ code-points, those won't be interoperable between ISPs, there is an ongoing proposal to solve this problem by building consistent code-points~\cite{diffservIETF}. Audio/video packets will be marked as priority using DSCP mappings with audio being more important than video or data~\cite{qosWebRTCIETF}. 
